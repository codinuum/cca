...
from cca.factutil.rdf import Resource, Predicate, Literal
import cca.factutil.range
...
class FactExtractor(object):
    def __init__(self, graph, conf, method='odbc', pw=VIRTUOSO_PW, port=VIRTUOSO_PORT):
        self._graph = graph
        self._conf = conf
        self.metrics = ""
...
class Finder(object):
    def __init__(self, qdir, queries, base_dir, proj_id, 
        ...
        self._cache_base_dir = os.path.join(base_dir, 'work.diffts', proj_id+'.fact')
        if conf == None:
        ...
        if extra_fact_extractor:
            self._extra_fact_extractor = extra_fact_extractor(self._graph_uri, self._conf,
    ...
    def get_ver_dir(self, verURI):
        return self._conf.get_ver_dir(self._conf.versions[self.get_vindex(verURI)])
    ...
    def get_version_pair(self, ent0, ent1):
        ...
        if pair == None:
            logger.warning('cannot get version pair for "%s" and "%s"' % (ent0, ent1))
        else:
    ...
    def get_srctree_pair_ent(self, ver0, ver1):
        ...
        n = len(l)
        if n == 0:
            logger.warning('SourceTreePair not found for "%s" and "%s"' % (ver0, ver1))
        elif n == 1:
            return l[0]
        else:
            logger.warning('multiple SourceTreePair found for "%s" and "%s":\n%s' % (ver0, ver1, '\n'.join(l)))
            return l[0]
    ...
    def get_srctree_ent(self, ver):
        ...
        n = len(l)
        if n == 0:
            logger.warning('SourceTree not found for "%s"' % ver)
        elif n == 1:
            return l[0]
        else:
            logger.warning('multiple SourceTree found for "%s":\n%s' % (ver, '\n'.join(l)))
            return l[0]
    ...
    def _get_version(self, ent, direction):
        ...
        if ver == None:
            logger.warning('cannot get version of "%s"' % ent)
        else:
            if direction >= 0: #latest
    ...
    def get_source_path(self, fid, verURI):
        ...
        if n == 1:
            ...
            logger.warning('multiple paths found for "%s"@"%s":\n%s' % (fid.encode(), 
        ...
        if path == None and not self._conf.include:
            logger.warning('path not found for "%s"@"%s"' % (fid.encode(), get_localname(verURI)))
    ...
    def get_source_url(self, fid, verURI, path=None):
        ...
        if self._conf.is_vkind_gitrev():
            ...
            if self._conf.gitweb_proj_id:
                url = '/gitweb/?p=%s;a=blob_plain;f=%s;h=%s;hb=%s' % (self._conf.gitweb_proj_id, 
                                                                      p, 
                                                                      fd, 
                                                                      get_localname(verURI))
            else:
                url = pathname2url(p)
                logger.warning('cannot get source url: fd="%s" verURI="%s"' % (fd, verURI))
        else:
            try:
                if p:
                    url = os.path.join(self.get_ver_dir_r(verURI), p)
            except Exception as e:
                logger.warning('cannot get source URL for fid=%s verURL=%s path=%s: %s' % (fid, verURI, path, e))
        return url
    ...
    def setup_metric_tbl(self, vpairs):
        ...
        if self._extra_fact_extractor:
            for (x, y) in vpairs:
                m = self._extra_fact_extractor.get_metric(get_localname(x), get_localname(y))
                if m:
    ...
    def sort_ver_pairs(self, vpairs):
        if vpairs:
            if self._metric_tbl:
                ...
                vpairs.sort(key=lambda p: self._metric_tbl[p]['value'], reverse=r)

            else:
                uris = self._conf.versionURIs
                vpairs.sort(key=lambda x_y: (uris.index(x_y[0]), uris.index(x_y[1])))
    ...
    def _show_stat(self, f, count_tbl, cov):
        ...
        f.write('<span class="term">Found:</span> %d change patterns (%d instances)<br/>\n' % (nchange_pats, total))

        if cov:
            f.write('<span class="term">Coverage:</span> %d/%d=%3.2f<br/>\n' % cov)
        ...
        for (change_pat, c) in count_items:
            ...
            f.write('<tr><td><span class="name">%s</span></td><td align="right">%d</td>' % (change_pat, c))
    ...
    def reduce_chgs(self, chgs):
        ...
        for chg in chgs:
            ...
            for (e0, e1, c, ct) in prune:
                if ch in ('deleted from', 'pruned from', 'weak removal') and e0.contains(ent0) and e0 != ent0:
                    b = False
                    break
            if b:
                for (e0, e1, c, ct) in graft:
                    if ch in ('inserted into', 'grafted onto', 'weak addition') and e1.contains(ent1) and e1 != ent1:
                        b = False
                        break
        ...
        logger.debug('|prune|=%d |graft|=%d |reduced|=%d' % (len(prune), len(graft), len(reduced)))
        ...
        for lang in self._result.keys():
            ...
            for (ver0, ver1) in ver_pairs:
                ...
                if self.extract_fact(lang) and self._extra_fact_extractor:
                    ...
                    self._extra_fact_extractor.extract_srctree_fact(fact, sent0, sent1, ver0, ver1)
                ### END EXTRACT FACT
                ...
                f_summary.write('<h3>Between "%s" and "%s"</h3>\n' % (aver0, aver1))
                ...
                if self._metric_tbl:
                    ...
                    met = '<span class="term">%s:</span> %s%s<br />\n' % (self._extra_fact_extractor.metrics, m['value'], copts_str)
                    f_summary.write(met)

                other_info = self._extra_fact_extractor.get_other_info(ver0, ver1)
                if other_info:
                    for k in other_info.keys():
                        f_summary.write('<span class="term">%s:</span> %s<br />\n' % (k, other_info[k]))


                f_sub.write('<h3>Patterns found between "%s" and "%s"</h3>\n' % (aver0, aver1))
                ...
                logger.info('%d instances found between "%s" and "%s"' % (ninstances, lver0, lver1))

                if self._limit != None:
                    if ninstances > self._limit:
                        reduced_count = 0
                        reduced_instances = random.sample(instances, self._limit)
                        logger.info('--> randomly sampled %d instances' % len(reduced_instances))
                        ...
                        logger.info('--> %d instances selected' % reduced_count)
                ...
                for fd_pair in fd_tbl.keys():
                   ...
                   for g in g_tbl.keys():
                       r_tbl = g_tbl[g]
                       for change_pat in r_tbl.keys():
                           count_tbl[change_pat] = count_tbl.get(change_pat, 0) + len(r_tbl[change_pat].keys())
                ...
                link = '<a href="%s" target="right">Show Detail</a>\n' % (os.path.basename(path_sub))
                ...
                for (fid0, fid1) in fd_tbl.keys():
                    ...
                    f_sub.write('<h4>[%d] %s <br/> - %s</h4>\n' % (file_count, path0, path1))
                    f_sub.write('<ul>\n')
                    ...
                    for g in g_tbl.keys():
                        ...
                        for change_pat in r_tbl.keys():
                            ...
                            for k in i_tbl.keys():
                                ...
                                chgpat_node = None
                                if file_pair_node:
                                    chgpat_node = mkchgpat(ent0, ent1, change_pat)
                                    chg_ty_node = self.mkchgtype(lang, change_pat)
                                    ps = self.get_predicates(lang)
                                    fact.add(file_pair_node, ps.p_chgpat, chgpat_node)
                                    fact.add(chgpat_node, ps.p_filepair, file_pair_node)
                                    fact.add(chgpat_node, P_TYPE, chg_ty_node)
                                    fact.add(chgpat_node, self.mkchgpatpred(lang, var0), ent0)
                                    fact.add(chgpat_node, self.mkchgpatpred(lang, var1), ent1)
                                ### END EXTRACT FACT
                                ...
                                ess0 = self.get_essentials_RM(change_pat, ent0, ent1)
                                ess1 = self.get_essentials_AD(change_pat, ent0, ent1)
                                ess01 = self.get_essentials_MP(change_pat, ent0, ent1)
                                ...
                                def mk_ess_tbl(ess, ess_tbl):
                                    for (v, es) in ess.items():
                                        ...
                                        for e in es:
                                            ofs = None
                                            if m:
                                                ofs = (mkofs(e[0], v[0]), mkofs(e[1], v[1]))
                                            else:
                                                ofs = mkofs(e, v)
                                            ...
                                            if ofs:
                                                ofss = []
                                                if m:
                                                    fd = (e[0].get_file_id(), e[1].get_file_id())
                                                else:
                                                    fd = e.get_file_id()
                                                try:
                                                    ofss = ess_tbl[fd]
                                ...
                                extra_pairs = self.get_extra_ent_pairs(change_pat, ent0, ent1)
                                ...
                                if chgpat_node:
                                    for (v0, v1, e0, e1) in extra_pairs:
                                        fact.add(chgpat_node, self.mkchgpatpred(lang, v0), e0)
                                        fact.add(chgpat_node, self.mkchgpatpred(lang, v1), e1)
                                ### END EXTRACT FACT
    ...
    def get_ent(self, uri):
        try:
            ...
            logger.error ('%s: uri=\"%s\"' % (e, uri))

    def get_uri(self, row, var):
        uri = row[var]
        if not uri:
            logger.warning('cannot get URI for "%s"' % var)
        return uri
        ...
        try:
            ...
            if per_ver:
                for (ver0, ver1) in self._conf.vURIpairs:
                    logger.info('  "%s"-"%s"' % (get_localname(ver0), get_localname(ver1)))
                    ...
                    for qvs, row in self._sparql.query(query):
                        ...
                        if uri0 == None or uri1 == None or skip:
                            continue
            ...
            for k0 in self._prim_chg_tbl.keys():
                ...
                logger.debug('  %s-%s:' % (get_localname(ver0), get_localname(ver1)))
                ...
                for k1 in self._prim_chg_tbl[k0].keys():
                    ...
                    logger.debug('    "%s"-"%s": %d' % (fid0.encode(), fid1.encode(), n))
    ...
    def is_removal(self, s):
        b = s in ('deleted or pruned', 'deleted from', 'pruned from', 'weak removal')
        logger.debug('%s --> %s' % (s, b))
        return b

    def is_addition(self, s):
        b = s in ('inserted or grafted', 'inserted into', 'grafted onto', 'weak addition')
        logger.debug('%s --> %s' % (s, b))
        return b

    def is_mapping_chg(self, s):
        b = s in ('changed to', 'modified', 'moved to', 'order changed', 'renamed')
        logger.debug('%s --> %s' % (s, b))
        return b
        ...
        try:
            ...
            for chg in chgs:
                ...
                if ent0 and not ent1:
                    ...
                    cond0 = ent0.contains(e0) and ent1.contains(e1) and self.is_mapping_chg(c)
    ...
    def find(self, force_per_ver=False, query_prec=False):
        ...
        for lang in self._queries.keys():
            if self._lang != None and lang != self._lang:
                continue
            ...
            for (q, var0, var1, extra, others, essential, inst_key, inst_key_is_one_to_one, is_complex, min_extra) in qdata:
                ...
                _query, per_ver = self.get_query(lang, q, force_per_ver=(force_per_ver and is_complex))
                ...
                if self._conf.vpairs:
                    ...
                    if per_ver:
                        ...
                        vpairs.append((self._conf.versionURIs[0], self._conf.versionURIs[1]))
                ...
                for (ver0, ver1) in vpairs:
                    ...
                    if per_ver:
                        query = self.add_ver_filter(_query, ver0, ver1)
                        sys.stdout.write('<%s -> %s> [%d/%d]' % (self.get_ver_name(ver0), 
                    ...
                    for row in rows:
                        ...
                        if query_prec and inst_key:
                            ...
                            if elemsl == []:
                                self._inst_elemsl_tbl[vp] = [set([x]) for x in ikey]
                            else:
                                ...
                                if any_match_mode:
                                    skip_flag = any([x in get_elems(i) for (i, x) in enumerate(ikey)])
                                elif all_match_mode:
                                    skip_flag = all([x in get_elems(i) for (i, x) in enumerate(ikey)])
                                else:
                                    vec = []
                    ...
                    for row in rows:
                        ...
                        if has_extra:
                            ...
                            logger.debug('extra_count=%d min_extra=%d' % (extra_count, min_extra))
                        ...
                        if fid0 == fid1:
                            logger.warning('illegal URI pair: %s and %s' % (uri0, uri1))
                        ...
                        for (v0, v1) in extra:
                            ...
                            if u0 and u1:
                                ...
                                self.add_extra_ent_pair(chg_name, (ent0, ent1), (v0, v1), (e0, e1))
                            ...
                            if u0:
                                ...
                                if self._change_enumeration:
                                    for (ver0_, ver1_) in version_pairs:
                                        self.remove_from_prim_chg_tbl(ver0_, ver1_, fid0, fid1, e0, None)
                        ...
                        for v1 in ess1:
                            ...
                            if u1:
                                ...
                                self.add_essential_AD(chg_name, (ent0, ent1), v1, e1)

                                if self._change_enumeration:
                                    for (ver0_, ver1_) in version_pairs:
                                        self.remove_from_prim_chg_tbl(ver0_, ver1_, fid0, fid1, None, e1)
                        ...
                        for (v0, v1) in ess01:
                            ...
                            if u0 and u1:
                                ...
                                self.add_essential_MP(chg_name, (ent0, ent1), (v0, v1), (e0, e1))

                                if self._change_enumeration:
                                    for (ver0_, ver1_) in version_pairs:
                                        self.remove_from_prim_chg_tbl(ver0_, ver1_, fid0, fid1, e0, e1)
