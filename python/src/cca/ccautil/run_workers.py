...
def dump_log(cmd_name, wid, sout_serr, wdir='.', timeout=TIMEOUT):
    ...
    while running:
        try:
            ...
            if nclosed >= nouts:
                running = False
                logger.info(f'[wid:{wid}] finished.')
...
#                engine_opt = '-e %s' % opt.engine
            ...
            cmd = f'{w_cmd} {arg} -c work -w {wid} {engine_opt} {target_dir}'
