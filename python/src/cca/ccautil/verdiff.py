...
from . import tp, diffts, srcdiff, project, AST, cca_factextractor
...
class TaskPool(tp.base):
    def __init__(
        ...
        self.cache_dir_base = os.path.abspath(os.path.join(self.basedir,
                                                           'work.diffts',
                                                           self.__conf.proj_id)
                                              )
        if self.load_fact:
    ...
    def gen_tasks(self):
        ...
        if self.__conf.vpairs:
            for (v1, v2) in self.__conf.vpairs:
                ...
                tasks.append((self.mkver_for_fact_by_name(v1),
                              self.mkver_for_fact_by_name(v2), ver1, ver2))

        else:
            vers = self.__conf.versions
            for i in range(self.__conf.nversions - 1):
                tasks.append((self.mkver_for_fact(i), self.mkver_for_fact(i+1),
                              vers[i], vers[i+1]))
    ...
    def task(self, args):
        ...
        if self.__conf.single_file_mode:
            ...
            r = srcdiff.diff_dirs(diff, dir1, dir2,
                                  usecache=(not self.clear_cache),
    ...
    def load(self, temp_file_size=DEFAULT_TEMP_FILE_SIZE):
        if self.load_fact and self.fact_into_virtuoso == '' \
           and self.fact_into_directory == '':
            ...
            scan_dir(self.cache_dir_base, diffts.fact_file_name_pat,
                     merger0.merge)
            merger0.load()

            logger.info('merging mapping fact')
            merger1 = FactMerger(loader, temp_file_size)
            scan_dir(self.cache_dir_base, diffts.mapfact_file_name,
                     merger1.merge)
            merger1.load()

            logger.info('merging change fact')
            merger2 = FactMerger(loader, temp_file_size)
            scan_dir(self.cache_dir_base, diffts.changefact_file_name,
                     merger2.merge)
            merger2.load()

            logger.info('merging cfg fact')
            merger3 = FactMerger(loader, temp_file_size)
            scan_dir(self.cache_dir_base, diffts.cfgfact_file_name,
                     merger3.merge)
            merger3.load()
...
def compute(load_fact=False,
    ...
    argparser.add_argument('--keep-going', dest='keep_going', default=False,
                           action='store_true',
                           help='ignore parse errors (default=%default)')

    argparser.add_argument('--sim', dest='use_sim', default=False,
                           action='store_true',
    ...
    argparser.add_argument('--delta', action='store_true',
                           dest='fact_for_delta', default=fact_for_delta,
                           help='output fact triples for delta')

    argparser.add_argument('--restrict', action='store_true',
                           dest='restrict_fact', default=restrict_fact,
                           help='restrict fact generation')

    argparser.add_argument('--changes', action='store_true',
                           dest='fact_for_changes', default=fact_for_changes,
                           help='output fact triples for changes')

    argparser.add_argument('--mapping', action='store_true',
                           dest='fact_for_mapping', default=fact_for_mapping,
    ...
    argparser.add_argument('--fact-size-thresh', type=int, metavar='N',
                           dest='fact_size_thresh',
                           default=diffts.DEFAULT_FACT_SIZE_THRESH,
...
#        diff = srcdiff.diffast
    ...
    if args.command == 'work':
        ...
        if load_fact:
            ...
            pool = TaskPool(args.basedir, working_dir, conf, False,
                            load_fact, fact_dir, ignore_unmodified,
                            restrict_fact,
                            fact_for_changes, fact_for_mapping, fact_for_ast,
                            fact_into_virtuoso, fact_into_directory,
                            fact_size_thresh,
