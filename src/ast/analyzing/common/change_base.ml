...
let find_from_or_into tree1 tree2 edits ?(mid_opt=None) tree' map_find node =
  let rec scan nd =
	...
	try
          let uid = nd#uid in
	  let uid' = map_find uid in

          let is_mov =
            if tree' == tree2 then
              edits#mem_mov12 uid uid'
            else
              edits#mem_mov12 uid' uid
          in
          if is_mov then
...
(*	    let into_nd = adjust_parent into_nd in *)
	...
	| _ -> []
      ...
      method private find_mid nd1 nd2 =
        try
          Some (edits#find_mid12 nd1#uid nd2#uid)
        with
          Not_found -> None
	...
	| Cmoved(nd1, nd2) ->
	    ...
	    let ent2 = mkent2 nd2 in
            ...
            let from_ent =
              mkent2 (self#find_from_or_into ~mid_opt tree2 uidmapping#find nd1)
            in
            let into_ent =
              mkent1 (self#find_from_or_into ~mid_opt tree1 uidmapping#inv_find nd2)
            in
	    let ctris =
	      if Triple.is_ghost_node category then
		[]
	...
	edits#finalize uidmapping (fun _ -> false)
...
*)
	...
	let get_list_of_node_and_new_ex tree nd ex =
	  ...
	  let exus = List.map (fun n -> n#uid) exns in
	  Array.fold_left
	    (fun l n ->
	      if not (List.mem n#uid exus) then
		let nex =
		  List.map Info.make
		    (List.filter
		       (fun exn ->
	...
	edits#iter
	  (fun ed ->
	    match ed with
	    | EO.Delete(_, _, info, ex) -> proc_del_ins EO.Tdel ed info ex
	    | EO.Insert(_, _, info, ex) -> proc_del_ins EO.Tins ed info ex

	    | EO.Relabel(_, (_, info1, _), (_, _, _)) ->
		let nd1 = Info.get_node info1 in
		if is_phantom nd1 then
		  self#remove_edit ed

	    | EO.Move(mid, kind, (_, info1, ex1), (_, info2, ex2)) ->
		let nd1 = Info.get_node info1 in
	...
	let for_rel_or_mov ed =
	  ...
	  | EO.Move(_, _, (_, info1, _), (_, info2, _)) ->
	      ...
	      let nd2 = Info.get_node info2 in
(*
	      if cond nd1 && cond nd2 then begin
		changes#add (Cmodified(nd1, nd2));
		self#set_used ed
	      end;
*)
	...
	edits#iter
	  (fun ed ->
	    match ed with
	    | EO.Delete(whole, _, info, _) ->
		let nd = Info.get_node info in
(*
		if cond nd && not whole then begin
		  try
		    let pnd = nd#initial_parent in
		    let puid' = uidmapping#find pnd#uid in
		    let pnd' = tree2#search_node_by_uid puid' in
		    let cands = ref [] in
		    Array.iter (fun c -> if cond c then cands := c::!cands) pnd'#children;
		    match !cands with
...
*)
	    ...
	    | EO.Insert(whole, _, info, _) ->
		let nd = Info.get_node info in
(*
		if cond nd && not whole then begin
		  try
		    let pnd = nd#initial_parent in
		    let puid' = uidmapping#inv_find pnd#uid in
		    let pnd' = tree1#search_node_by_uid puid' in
		    let cands = ref [] in
		    Array.iter (fun c -> if cond c then cands := c::!cands) pnd'#children;
		    match !cands with
...
*)
	...
	let parent_cond =
	  try
            ...
            let c =
              try
                mid <> edits#find_mid12 puid1 puid2
              with
                Not_found -> true
            in
	    uidmapping#has_mapping puid1 puid2 && c
	  with
	    _ -> false
	in
	...
	edits#iter_moves
	  (fun ed ->
	    match ed with
	    | EO.Move(mid, sub, (uid1, info1, ex1), (uid2, info2, ex2)) ->
	        if not (self#is_used ed) then begin
		  let nd1 = Info.get_node info1 in
		  let nd2 = Info.get_node info2 in
(*
 ...
 *)
	...
	edits#iter_relabels
	  (fun ed ->
	    match ed with
	    | EO.Relabel(_, (uid1, info1, _), (uid2, info2, _)) ->
		if not (self#is_used ed) then begin
		  let nd1 = Info.get_node info1 in
		  let nd2 = Info.get_node info2 in
		  let is_mov = edits#mem_mov12 uid1 uid2 in
	...
	edits#iter_moves
	  (fun ed ->
	    match ed with
	    | EO.Move(mid, sub, (_, info1, _), (_, info2, _)) ->
		let nd1 = Info.get_node info1 in
		let nd2 = Info.get_node info2 in
		if
		  cond nd1 && cond nd2 &&
	...
	let au2 = an2#uid in
	let au1 = uidmapping#inv_find au2 in
	let a1 = tree1#search_node_by_uid au1 in
