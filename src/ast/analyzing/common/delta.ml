...
module Edit = struct
  ...
  let op_of_editop (eop : 'data eop_t) =
    let op =
      match eop with
  ...
  class ['node, 'tree] seq
    ...
    let edit_list =
      ...
      if options#minimize_delta_flag || more then
        ...
        List.map op_of_editop edit_seq#content
    in
    object (self)
	  ...
	  List.fast_sort
        ...
        in
        let sorted_edits =
          let dels, inss_movs, rels = ref [], ref [], ref[] in
          self#iter
	    (fun edit ->
	      match edit with
	      | Relabel _ -> rels := edit::!rels
	      | Delete _  -> dels := edit::!dels
	      | Insert _ | Move _ -> inss_movs := edit::!inss_movs
	    );
      ...
      method get_subpath tree tree' nmap' ?(ins_point_opt=None) k bn' =
        ...
        let is_excluded =
          ...
          let get_grp x' =
            ...
            in
            DEBUG_MSG "ss=[%a]" nsps ss;
            let grp = List.filter (fun tn -> List.exists (fun s -> tn == s || is_ancestor tn s) ss) top_nodes in
          ...
          let extra_base_cond x' =
            ...
            if
              ...
              try
                ...
                Hashtbl.mem edit_parent_tbl xk || Hashtbl.mem parent_key_tbl xk
              with _ -> false
            then
              false
            else
            ...
            array_range_exists
              ...
              try
                let xk = self#find_key x'#uid in
                let a, (pt, ps) =  Hashtbl.find ancto_tbl xk in
                pt#key_opt = None && not (Hashtbl.mem edit_parent_tbl xk) && not (Hashtbl.mem parent_key_tbl xk)
              with _ -> false) ||
                       ...
                       (fun y' ->
                         let b =
                           ...
                           List.exists
                             ...
                             ) (List.map nmap' (if is_stable' y' then [y'] else get_p_descendants is_stable' y'))
                         in
                         if b then
      ...
      method get_adjusted_path
        ...
        get_adjusted_path ~get_group ~group_heads ~get_iparent_opt
          ...
          is_simple_ins is_excluded is_stable root rel_path
                          ...
                          let not_to_be_reversed =
                            array_range_exists
                              (fun c ->
                                ...
                                array_range_exists (fun c1 -> List.memq c1 rest) ca (c#initial_pos + 1) (len - 1)
                ...
                List.iteri
                  (fun i sn ->
                    ...
                    then begin
                      ...
                      DEBUG_MSG "lift cand: i=%d mem=%a" i nps mem;
                          ...
                          if all_cond then begin
                            Array.iter
                              (fun w ->
                                DEBUG_MSG "w=%a" nps w;
                                (*let moveon x = not (is_stable x) in*)
                                let moveon x = not (self#is_canceled_stable_node x && x#initial_pos = 0) in
                                let context_cond =
                                  ((not (is_stable w) ||
                                  let w' = nmap w in
                                  List.exists (fun t' -> t' == w' || is_ancestor t' w') top_nds')) &&
                                  (is_stable w && List.memq (nmap w) top_nds' ||
                                  let lss = get_p_left_nodes ~moveon is_stable_ w nd in
                                  DEBUG_MSG "lss=[%a]" nsps lss;
                                  if lss <> [] then begin
                                    let rss = get_p_right_nodes ~moveon is_stable_ w nd in
      ...
      method private get_conflicts ?(ancto_opt=None) ?(parent_ins_target_opt=None)
        ...
        BEGIN_DEBUG
          ...
          | Some x -> DEBUG_MSG "parent_ins_target=%a" nps x
                  ...
                  | Some k -> begin
                      ...
                      if List.mem k !ks then
                        ...
                        try
                          ...
                          not
                            (array_range_exists
                               (fun x -> x == nd || is_ancestor x nd) a#initial_children p (p+nb-1)) ||
                          array_range_exists
                            (fun x ->
                              is_ancestor x nd &&
                ...
                | Some lsn', _, Some rsn' -> begin
                    ...
                    let rsn', rsn, lr_sns', anc' =
                      ...
                      | r' :: (_::_) when
                          ...
                          try lsn'#initial_parent#initial_children.(lsn'#initial_pos+1) == rsn' with _ -> false
                        -> begin
                          let r = nmap' r' in
                    ...
                    let context_cond =
                      let b = self#has_parent_path lsn' && self#has_parent_path rsn' in if b then DEBUG_MSG "!!!!!"; b ||
                      ...
                      | Some (lr, rr) -> begin
                          ...
                          let a' =
                            ...
                            if is_ancestor a0' a' || a0' == a' then
                              ...
                              | [] -> lsn'#initial_parent
                              | rsn0'::_ ->
                                  let a0', _ = self#get_latest_common_ancestor tree' [lsn'; rsn0'] in
                                  a0'
                          in
                          DEBUG_MSG "pnd=%a" nps pnd;
                          DEBUG_MSG "a'=%a" nps a';
                        ...
                        | Some (idx, wall) -> begin
                            ...
                            let w_grp_cond =
                              if is_stable wall then
                                ...
                                DEBUG_MSG "a'=%a wall'=%a" nps a' nps wall';
                      ...
                      try
                        ...
                        then begin
                          ...
                          try
                            ...
                            Array.exists (fun n -> n != r && pred ~upper:true n) rca
                          with _ -> false
                        then begin
                          path_to#set_upstream 1;
                          path_to#set_key_opt k_opt;
	...
	let dump_content tree root excluded =
        ...
        let reg_edit_parent tbl parent_key_opt mkkey =
          ...
          | Some k -> Hashtbl.add tbl (mkkey()) k
          | None -> ()
        in
	  ...
	  | Relabel(nd1, excluded1, nd2, excluded2) -> begin
          ...
          | Move(mid, _, nd1, excluded1, nd2, excluded2) -> begin
              ...
              (*let excluded1 =
                List.filter (fun x -> not (self#is_canceled_stable_node x)) _excluded1
              in
              let excluded2 =
                List.filter (fun x -> not (self#is_canceled_stable_node x)) _excluded2
              in*)
                      ...
                      (*if self#is_stable1 n1 then
                        ...
                        | Some k -> begin
                            ...
                            try
                              ...
                              let conflicts', _, _, _ =
                                self#get_conflicts self#is_stable1 self#is_stable2 tree1 tree2 nmap1 nmap2 n1 n2
                              in
                              conflicts' <> []
                            with
